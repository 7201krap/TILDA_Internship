...
if first_run == True:
    ...
    plt.ylabel('Fitness')
    plt.title('Fitness History of Maze SM-G-SO Mutation')
    plt.grid()
    plt.legend()
    plt.savefig('../results/Maze ga sm-g-so')
    plt.show()

    # save the best model
    fitnesses = [calculate_fitness(network, env, MAX_EP) for network in tqdm(population, desc="Calculating fitnesses")]
    torch.save(population, '../results/maze_gaO_smg-so.pth')

else:
    seed_setter(seed_value=0)

    loaded_population = torch.load('../results/maze_gaO_smg-so.pth')
