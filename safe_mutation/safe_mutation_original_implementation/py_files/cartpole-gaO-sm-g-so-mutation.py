...
def perturb_parameters(network, weight_clip, n_episodes):
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            perturbation = np.clip(delta / scaling, -weight_clip, weight_clip)
            perturbation[mask] = np.clip(delta[mask] / scaling[mask], -weight_clip, weight_clip)
...
if first_run == True:
    ...
    plt.ylabel('Fitness')
    plt.title('Fitness History of CartPole SM-G-SO Mutation')
    plt.grid()
    plt.legend()
    plt.ylim(top=700)
    plt.savefig('../results/cartpole ga sm-g-so')
    plt.show()
    ...
    torch.save(best_network.state_dict(), '../results/cartpole_gaO_smg-so.pth')
    ...
    new_network.load_state_dict(torch.load('../results/cartpole_gaO_smg-so.pth'))
