...
def perturb_parameters(network, weight_clip, n_episodes):
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            gradient[mask] /= torch.sqrt(((gradient[mask]**2).sum() + 1e-10))
...
if first_run == True:
    ...
    plt.ylabel('Fitness')
    plt.title('Fitness History of CartPole SM-G-FO Mutation')
    plt.grid()
    plt.legend()
    plt.ylim(top=700)
    plt.savefig('../results/cartpole ga sm-g-fo')
    plt.show()
    ...
    torch.save(best_network.state_dict(), '../results/cartpole_gaO_smg-fo.pth')
    ...
    new_network.load_state_dict(torch.load('../results/cartpole_gaO_smg-fo.pth'))
