...
def main(POPULATION_SIZE, GENERATIONS, ELITISM, TOURNAMENT_SIZE, MUTATION_STRENGTH, MUTATION_RATE):
    ...
    population = [PolicyNetwork(24, 4) for _ in range(POPULATION_SIZE)]
...
if first_run == True:
    ...
    plt.ylabel('Fitness')
    plt.title('Fitness History of Bipedal Classic Mutation')
    plt.grid()
    plt.legend()
    ...
    new_network = PolicyNetwork(24, 4)

    # Then you can load the weights
    new_network.load_state_dict(torch.load('../results/bipedal_gaO_control.pth'))
