...
def perturb_parameters(network, weight_clip, n_episodes):
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            perturbation = np.clip(delta / torch.sqrt(((jacobian**2).sum() + 1e-10)), -weight_clip, weight_clip)
            perturbation[mask] = np.clip(delta[mask] / torch.sqrt(((jacobian ** 2).sum() + 1e-10)), -weight_clip, weight_clip)
...
if first_run == True:
    ...
    plt.ylabel('Fitness')
    plt.title('Fitness History of CartPole SM-G-SUM Mutation')
    plt.grid()
    plt.legend()
    plt.ylim(top=700)
    plt.savefig('../results/cartpole ga sm-g-sum')
    plt.show()
    ...
    torch.save(best_network.state_dict(), '../results/cartpole_gaO_sm-g-sum.pth')

else:
    # load the best model
    # First, you have to create an instance of the same model architecture
    new_network = PolicyNetwork(4, 2)

    # Then you can load the weights
    new_network.load_state_dict(torch.load('../results/cartpole_gaO_sm-g-sum.pth'))
